...
setwd("/Shared/ssrivastva/pie/mixef/result/")
...
for (ii in 1:10) {
  ...
  mcmcFix[[ii]] <- dat$samples[ , 1:4]
  mcmcRan[[ii]] <- dat$samples[ , c(5:7, 9:10, 13)]
...
for (ii in 1:10) {
    ...
    mlRan[[ii]] <- dat$ranefCov[lower.tri(dat$ranefCorr, diag = TRUE)]
...
for (ii in 1:10) {
  ...
  for (jj in 1:20) {
    ...
    pieFix[[ii]][[jj]] <- dat$samples[ , 1:4]
    pieRan[[ii]][[jj]] <- dat$samples[ , c(5, 6, 7, 9, 10, 13)]
...
for (ii in 1:10) {
  ...
  for (jj in 1:20) {
    tmp[[jj]] <- do.call(cbind, lapply(split(pieRan[[ii]][[jj]], col(pieRan[[ii]][[jj]])),
                                       function(x) quantile(x, prob = seq(0, 1, by = 0.01))))
  }
...
mlCorr <- lapply(mlRan, function(x) {
    x / c(1, sqrt(x[1] * x[4]), sqrt(x[1] * x[6]), 1, sqrt(x[4] * x[6]), 1)
})
...
for (jj in 1:6) {
    ...
    resRan[["Scot"]][[jj]] <- list()
    for (ii in 1:10) {
        rr <- range(c(mcmcRan[[ii]][ , jj], vbRan[[ii]][ , jj], pieRanEst[[ii]][ , jj], waspRan[[ii]][ , jj]))
        bw1 <- dpik(mcmcRan[[ii]][ , jj], range.x = rr)
        bw2 <- dpik(vbRan[[ii]][ , jj], range.x = rr)
        bw3 <- dpik(pieRanEst[[ii]][ , jj], range.x = rr)
        bw4 <- dpik(waspRan[[ii]][ , jj], range.x = rr)
        bw5 <- dpik(scotRan[[ii]][ , jj], range.x = rr)
        bw6 <- dpik(xingRan[[ii]][ , jj], range.x = rr)
        dens1 <- bkde(mcmcRan[[ii]][ , jj], bandwidth = bw1, range.x = rr)
        dens2 <- bkde(vbRan[[ii]][ , jj], bandwidth = bw2, range.x = rr)
        dens3 <- bkde(pieRanEst[[ii]][ , jj], bandwidth = bw3, range.x = rr)
        dens4 <- bkde(waspRan[[ii]][ , jj], bandwidth = bw4, range.x = rr)
        dens5 <- bkde(scotRan[[ii]][ , jj], bandwidth = bw5, range.x = rr)
        dens6 <- bkde(xingRan[[ii]][ , jj], bandwidth = bw6, range.x = rr)
...
for (jj in 1:6) {
    ...
    for (ii in 1:10) {
        rr <- range(c(mcmcCorr[[ii]][ , jj], vbCorr[[ii]][ , jj], pieCorrEst[[ii]][ , jj], waspCorr[[ii]][ , jj]))
        bw1 <- dpik(mcmcCorr[[ii]][ , jj], range.x = rr)
        bw2 <- dpik(vbCorr[[ii]][ , jj], range.x = rr)
        bw3 <- dpik(pieCorrEst[[ii]][ , jj], range.x = rr)
        bw4 <- dpik(waspCorr[[ii]][ , jj], range.x = rr)
        bw5 <- dpik(scotCorr[[ii]][ , jj], range.x = rr)
        bw6 <- dpik(xingCorr[[ii]][ , jj], range.x = rr)
        dens1 <- bkde(mcmcCorr[[ii]][ , jj], bandwidth = bw1, range.x = rr)
        dens2 <- bkde(vbCorr[[ii]][ , jj], bandwidth = bw2, range.x = rr)
        dens3 <- bkde(pieCorrEst[[ii]][ , jj], bandwidth = bw3, range.x = rr)
        dens4 <- bkde(waspCorr[[ii]][ , jj], bandwidth = bw4, range.x = rr)
        dens5 <- bkde(scotCorr[[ii]][ , jj], bandwidth = bw5, range.x = rr)
        dens6 <- bkde(xingCorr[[ii]][ , jj], bandwidth = bw6, range.x = rr)
...
for (jj in 1:4) {
    ...
    resFix[["Xing"]][[jj]] <- list()
    for (ii in 1:10) {
        rr <- range(c(mcmcFix[[ii]][ , jj], vbFix[[ii]][ , jj], pieFixEst[[ii]][ , jj], waspFix[[ii]][ , jj], mlFix[[ii]][ , jj]))
...
tblCorr <- matrix(paste(format(t(round(apply(accCorr, 3, colMeans), 2)), nsmall = 2),
      paste0("(", format(t(round(apply(accCorr, 3, colSds), 2)), nsmall = 2), ")")
      ), 5, 6)
...
for (ii in 1:6) {
    for (jj in 1:6) {
        ciRanTbl[jj , ii] <- paste0("(",
                               format(round(tb2["lb", ii, jj], 2), nsmall = 2), ", ",
                               format(round(tb2["ub", ii, jj], 2), nsmall = 2), ")")
    }
}
...
ciFix <- array(0.0, dim = c(10, 4, 2, 7),
               dimnames = list(paste0("cv", 1:10),
                               paste0("dim", 1:4),
                               c("lb", "ub"),
...
t(round(apply(accRan, 3, colMeans), 2))
t(round(apply(accRan, 3, colSds), 2))

ffm <- t(round(apply(accFix, 3, colMeans), 2))
ffs <- t(round(apply(accFix, 3, colSds), 2))
resFix <- matrix("0", 4, 4)
for (ii in 1:4) {
  for (jj in 1:4) {
      resFix[ii , jj] <- paste0(ffm[ii, jj], " (", ffs[ii, jj], ", ", ffs[ii, jj], ")")
  }
}

xtable::xtable(resFix)

rrm <- t(round(apply(accRan, 3, colMeans), 2))
rrs <- t(round(apply(accRan, 3, colSds), 2))
resRan <- matrix("0", 3, 6)
for (ii in 1:3) {
  for (jj in 1:6) {
      resRan[ii , jj] <- paste0(rrm[ii, jj], " (", rrs[ii, jj], ", ", rrs[ii, jj], ")")
  }
}

xtable::xtable(resRan[ , c(1, 4, 6, 2, 3, 5)])

fnames <- c(1:4)
for (jj in 1:4) {
  pdf(paste0("~/pie/mixef/result/img/fix_", fnames[jj], ".pdf"), 45, 15)
  par(mfrow = c(2, 5))
  par(cex = 1)
  ...
  for (ii in 1:10) {
    ...
    if (ii == 1) {
      legend("topright", c("MCMC", "ML", "VB", "WASP", "PIE"), col = colors[c(1, 5, c(2, 4, 3))],
...
rnames <- c("1_1",  "2_2", "3_3")
cnames <- c("2_1",  "3_1", "2_3")
varMap <- c(1, 4, 6)
covMap <- c(2, 3, 5)

for (jj in 1:3) {
  pdf(paste0("~/pie/mixef/result/img/var_", rnames[jj], ".pdf"), 40, 15)
  par(mfrow = c(2, 5))
  par(cex = 1)
  par(mar = c(0, 0, 0, 0), oma = c(4.5, 4.5, 0.4, 0.4))
  par(tcl = -0.02)
...
for (jj in 1:3) {
  pdf(paste0("~/pie/mixef/result/img/cov_", cnames[jj], ".pdf"), 40, 15)
  par(mfrow = c(2, 5))
  par(cex = 1)
  par(mar = c(0, 0, 0, 0), oma = c(4.5, 4.5, 0.4, 0.4))
  par(tcl = -0.02)
...
pdf("~/pie/mixef/result/img/mixef_time.pdf", 8, 8)
par(cex = 1)
par(mar = c(0, 0, 0, 0), oma = c(3, 5.5, 0.2, 0.2))
par(tcl = -0.02)
par(mgp = c(2, 0.6, 0))
boxplot(runTime, ylab = NA, axes = FALSE, lwd = 2, ylim = c(0, 5))
grid(lwd=3)
...
mtext(format(seq(0, 5, by = 0.5)), at = seq(0, 5, by = 0.5), side = 2, line = 0.3, las = 1, cex = 2)
...
for (ii in 1:4) {
  for (jj in 1:5) {
      res[jj , ii] <- paste0("(",
         round(colMeans(do.call(rbind, lapply(est[[jj]], function(x) x[, 1])))[ii], 2), ", ",
         round(colMeans(do.call(rbind, lapply(est[[jj]], function(x) x[, 2])))[ii], 2), ")")
  }
}
...
pdf(paste0("~/pie/mixef/result/img/sim_fixef.pdf"), 40, 10)
par(mfrow = c(1, 4))
par(cex = 1)
par(mar = c(0, 4, 0, 0), oma = c(4.5, 1.5, 0.4, 0.4))
par(tcl = -0.02)
par(mgp = c(2, 0.6, 0))
for (jj in 1:4) {
  for (ii in 5) {
    ...
    lines(resFix[["WASP"]][[jj]][[ii]]$x, resFix[["WASP"]][[jj]][[ii]]$y, col = colors[4], lwd = 6)
    lines(resFix[["PIE"]][[jj]][[ii]]$x, resFix[["PIE"]][[jj]][[ii]]$y, col = colors[3], lwd = 6)
    mtext(bquote(beta[.(jj)]), side = 3, line = -3.5, adj = 0.1, cex = 4)
    if (jj == 1) {
      legend("topright", c("MCMC", "ML", "VB", "WASP", "PIE"), col = colors[c(1, 5, 2, 4, 3)],
             lty = "solid", lwd = 6, bty = "n", cex = 2.5)
    }
  }
}
dev.off()

rnames <- c("11",  "22", "33", "21",  "31", "23")
varMap <- c(1, 4, 6, 2, 3, 5)

pdf(paste0("~/pie/mixef/result/img/sim_ranef.pdf"), 50, 10)
par(mfrow = c(1, 6))
par(cex = 1)
par(mar = c(0, 4, 0, 0), oma = c(4.5, 1.5, 0.4, 0.4))
par(tcl = -0.02)
...
for (jj in 1:6) {
  for (ii in 5) {
    rr0 <- (range(unlist(lapply(resRan[["MCMC"]][[varMap[jj]]], function(x) x$y)), unlist(lapply(resRan[["PIE"]][[varMap[jj]]], function(x) x$y)), unlist(lapply(resRan[["WASP"]][[varMap[jj]]], function(x) x$y))))
    rr <- c(round(rr0[1]), ceiling(rr0[2]) + 1)
    plot(resRan[["MCMC"]][[varMap[jj]]][[ii]]$x, resRan[["MCMC"]][[varMap[jj]]][[ii]]$y, col = colors[1], ylab = NA, xlab = NA, axes = FALSE, type = "l", ylim = rr)
    xxlab <- axTicks(1)
    yylab <- axTicks(2)
    axis(side = 2, tck = -0.01, lwd = 3, cex = 2, labels = NA)
    ...
    lines(resRan[["PIE"]][[varMap[jj]]][[ii]]$x, resRan[["PIE"]][[varMap[jj]]][[ii]]$y, col = colors[3], lwd = 6)
    abline(v = mlRan[[ii]][[varMap[jj]]], col = colors[5], lwd = 6)
    mtext(bquote(Sigma[.(rnames[jj])]), side = 3, line = -3.5, adj = 0.1, cex = 4)
    if (jj == 1) {
      legend("topright", c("MCMC", "ML", "VB", "WASP", "PIE"), col = colors[c(1, 5, 2, 4, 3)],
             lty = "solid", lwd = 6, bty = "n", cex = 2.5)
    }
...
pdf("~/pie/mixef/result/img/sim_ran_corr_box.pdf", 30, 10)
par(mfrow = c(2, 3))
par(cex = 1)
...
for (dd in 1:length(idxran)) {
    ...
    if (dd <=3) {
        mtext(bquote(Sigma[.(rnames[dd])]), side = 3, line = 0, adj = 0.5, cex = 3)
    } else {
        mtext(bquote(rho[.(rnames[dd])]), side = 3, line = 0, adj = 0.5, cex = 3)
    }
}
dev.off()
...
pdf("~/pie/mixef/result/img/sim_ran_box.pdf", 30, 10)
par(mfrow = c(2, 3))
par(cex = 1)
par(mar = c(2, 4, 3, 1), oma = c(1, 1, 0.4, 0.4))
par(tcl = -0.02)
...
pdf("~/pie/mixef/result/img/sim_fix_box.pdf", 30, 6)
par(mfrow = c(1, 4))
par(cex = 1)
par(mar = c(2, 5, 3, 0.5), oma = c(1, 1, 0.4, 0.4))
par(tcl = -0.02)
par(mgp = c(2, 0.6, 0))
for (dd in 1:length(idxfix)) {
    rr <- range(unlist(lapply(datFix, function(x) x[ , dd]))) + c(-0.001, 0.001)
    ...
    axis(side = 2, tck = -0.01, labels = NA, lwd = 1.5)
    mtext(yy, at = yy, side = 2, line = 0.3, las = 1, cex = 2)
    mtext(bquote(beta[.(dd)]), side = 3, line = 0, adj = 0.5, cex = 4)
}
dev.off()
