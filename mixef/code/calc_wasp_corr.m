%dirp = '/datacommons/carin/ss602/mixef/' <-- note the / at the end ...
% $$$ keys=c(5, 6, 7, 9, 10, 13) 
% $$$ [1] "covRanef[1,1]" "covRanef[2,1]" "covRanef[3,1]" "covRanef[2,2]"
% $$$ [5] "covRanef[3,2]" "covRanef[3,3]"
function calc_wasp_corr(dd, nsub, ndim, keys, dirp)
...
for jj = 1:nsub
    ...
    corrs{jj, 1} = covs{jj, 2} ./ sqrt(covs{jj, 1} .* covs{jj, 4});
    corrs{jj, 2} = covs{jj, 3} ./ sqrt(covs{jj, 1} .* covs{jj, 6});
    corrs{jj, 3} = covs{jj, 5} ./ sqrt(covs{jj, 4} .* covs{jj, 6});        
end
...
for dims = 1:3
    subsetPost = {};    
    for jj = 1:nsub
        subsetPost{jj} = corrs{jj, dims}(randi([1 1000], 200, 1));
    end         
    ...
    save(strcat(dirp, 'result/wasp/corr_fit_', num2str(dims), '_rep_', num2str(dd), '.mat'), 'tmats','avec', 'subsetPost', 'overallPost');    
    summ = [overallPost avec];
    csvwrite(strcat(dirp, 'result/wasp/corr_', num2str(dims), '_rep_', num2str(dd),  '.csv'), summ);

    disp(['evaluating sim ' num2str(dd) '...' ' done with dim ' num2str(dims) '... ']);    
end
csvwrite(strcat(dirp, 'result/wasp/corr_times_rep_', num2str(dd),  '.csv'), rtime);    
